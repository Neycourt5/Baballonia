...
- **`bin\Baballonia-v13\Baballonia.Desktop.exe`**, ProductVersion `1.0.0+1306fe6…` — the last commit
  that changed compiled code. Anything after it here is documentation only, which cannot change the
  binary; chasing an exact match against a docs commit would mean rebuilding forever. What matters
  is that no *code* is in the build that is not in the history, which is what v12 could not claim:
  it was published from a dirty tree and reported the base revision.
