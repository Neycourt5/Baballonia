...
public class MatToFloatTensorConverter : IImageConverter
{
    ...
    public void Convert(Mat input, DenseTensor<float> outTensor)
    {
        ...
        try
        {
            // The transformer already produces the model's input size, so this is normally a resize
            // from a size to itself - real work in OpenCV, on every frame of both pipelines.
            var target = new Size(outTensor.Dimensions[2], outTensor.Dimensions[3]);
            if (resultMat.Width != target.Width || resultMat.Height != target.Height)
                Cv2.Resize(resultMat, resultMat, target);
