...
public class ProcessingLoopService : IDisposable
{
    ...
    private readonly NeutralExpressionRamp _faceNeutralRamp = new(_ => 0f);
    private readonly NeutralExpressionRamp _eyeNeutralRamp = new(EyeOutputPostProcessor.NeutralForKey);
    ...
    private void EyeWorker()
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    _eyeNeutralRamp.Observe(eyeExpression);
                ...
                var neutral = _eyeNeutralRamp.Sample(_eyePipelineManager.IsTotalOutage);
                if (neutral != null)
                    ExpressionChangeEvent?.Invoke(new Expressions(null, neutral));
            }
            catch (Exception ex)
            {
                _logger.LogError("Unexpected exception in Eye Tracking pipeline; reconnecting: {}", ex);
                _eyePipelineManager.FaultEyeCameras();
    ...
    private void FaceWorker()
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    _faceNeutralRamp.Observe(faceExpression);
                    Interlocked.Increment(ref _metrics.FaceInferences);
                    ExpressionChangeEvent?.Invoke(new Expressions(faceExpression, null));
                    continue;
                }

                var neutral = _faceNeutralRamp.Sample(
                    _facePipelineManager.State == CameraState.Reconnecting);
                if (neutral != null)
                    ExpressionChangeEvent?.Invoke(new Expressions(neutral, null));
            }
            catch (Exception ex)
            {
                _logger.LogError("Unexpected exception in Face Tracking pipeline; reconnecting: {}", ex);
                _facePipelineManager.FaultCamera();
